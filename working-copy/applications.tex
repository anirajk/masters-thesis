...
We evaluate the improvement in throughput and efficiency to provide evidence to back the approach of bounded disorder.
We also make informed opinions on how Zero Copy might perform on column store designs for in-memory databases with co-operation from clients for materialising data.
...
Column stores use contigous memory locations to store adjacent columns. They have also been known 
to use Run-Length Encoding, Dictionary Encoding, Bit-Vector Encoding and more heavier schemes 
like Lempel-Ziv Encoding for yielding better compression which is one of the many value 
propositions of using a column oriented layout. We observed in the previous 
sections, Zero Copy offers promising performance if we had stable entries and used Zero Copy
 to transmit the results while responding to a range query. 
...
We argue for the possibility of a different materialisation strategy with more engagement from the 
...
column oriented formats varying width of columns according to the type of workload. Figure ~\ref{fig:rowstore-colstore}
...
\begin{enumerate}
\item Client co-operation; this scheme of transmitting results would need smart clients which are 
...
\item Low selectivity in queries; we have seen packing more data yields more savings and performance 
...
\item Well compressed metadata; the client assisted design would need the encoding to be transmitted 
