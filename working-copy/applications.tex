...
Since more databases have started to store all data in memory at all times, database designers have started to normalize 
aggressively and as a result, record sizes are trending smaller and 128~B records show
a more realistic depiction of record sizes in the future~\cite{fb-memcache,fb-workload,inmemoryworkload}. 
...
While transmitting at around ~5.9GB/s(near line rate for the Connect-X3\textregistered NIC),
we saw that the memory pressure on the system is close to 12GB/s which is half of what we evaluated our server's total available memory bandwidth.
...
On-Line Analytical Processing (OLAP) and hybrid workloads hasvealso fuelled a renewed interest in column stores~\cite{cstore,cstorevsrowstore} in recent years. The impact of 
modern NIC while designing column stores should also be discussed for completeness. This chapter introduces a hybrid approach between Zero Copy and 
Copy Out using a client assisted design and evaluates the improvement in throughput and efficiency to provide evidence to back the approach.
 We also make informed opinions on how Zero Copy might perform on column store designs for in-memory databases.
...
starving the server of memory bandwidth which could be utilised for doing useful work.
...
Zero Copy paradigm. This would impy an arrangement of small records where the total transmission size 
goes up helping the transmission throughput while being able to do Zero Copy to help reduce memory footprint. In order for this 
to work, we would also need a way of efficient update and synchronisation so that data being fetched for transmission remains 
...
Bw-Tree is a highly performant, ordered index employing lock and latch free concurrency and effective utilisation of caches in 
...
Column oriented in-memory databases poses unique challenges in interacting with a modern NIC. 
Column stores use contigous memory locations to store adjacent columns. Like we saw for the 
Bw-Trees, it should be easy to construct a large base page with all the stable entries and 
using Zero Copy to transmit the results while responding to a range query. 
This would help transmission throughput and efficient use of memory bandwidth. 
However, the key problem lies in the fact that end-users expect query processing and analytics on data 
to materialise results in row-order. It has been argued before that early materialisation of data
helps performance in column oriented databases~\cite{cstore-material}. If we were to discard these 
considerations, Zero Copy might appear to be of great benefit. Since the response is larger, they 
could be treated as one huge base page and any updates could be treated as delta records like we 
saw in Section~\ref{sec:deltarecords} which would allow us to saturate the NIC's throughput without 
compromising on memory bandwidth consumption.
...
\begin{itemize}
\item With the assumption of {\em co-operative clients}, we can continue to use Zero Copy and get better results 
while dealing with small, scattered records which are common in in-memory databases.
\item Advanced structures such as the Bw-Tree which allows no update in place and complete lock and latch freedom 
can be exploited for it's suitability with the NICs.
\item Column stores could also benefit from the hybrid approach; materialising results in row-order makes Zero Copy 
more complicated in modern NICs.
\end{itemize}
