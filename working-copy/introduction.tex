...
near line rate network transmission. 
...
memory bandwidth are of paramount. Our protocol will make the most efficient use of these resources
...
to provide fast log replay and jitter-free data movement in a cluster for reconfiguration and improved 
...
\item{\textbf{Evaluation of server impact during transmission}}: We measure uncore events that reveal impact on memory bandwidth,
 DDIO traffic and PCIe traffic in a system that's churning out heavy responses. We believe this is the first study that does an 
 in-depth analysis of server impact using uncore events for data transmissions near line rate.
