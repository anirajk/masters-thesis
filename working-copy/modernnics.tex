...
\begin{itemize}
\item Comparison of various RDMA primitives over ibverbs library; \cpp{SEND} is best suited for returning scattered records.
...
from a given location from the source. The receiver needs to be active and you can
 only gather one remote location in one call to the \cpp{ibv_post_send}. It could work 
 better if we were to pipeline multiple requests, but we are losing out on the 
...
\framebox[0.9\textwidth][c]{
  \parbox{0.7\textwidth}{
Of the various modes of transmissions, SEND is the most promising.
\par
 WRITE would offer similar performance but requires an active receiver with a registered buffer.
 \par
 READ is also client initiated. It could do well when pipelined but a single gather entry limits it's usability for benchmarking.}
}
...
\framebox[0.9\textwidth][c]{
  \parbox{0.7\textwidth}{
  Bigger transmissions saturate the NIC. Zero Copy is limited by S/G length. 
  \par
  Zero Copy is a clear winner for bigger chunks.
  \par
  For smaller chunks, if your records are scattered beyond a point (16 out of a 32 big list),
  it's better to use Copy Out}
  }
...
\framebox[0.9\textwidth][c]{
  \parbox{0.7\textwidth}{
  \memcpy ~takes up the biggest chunk of time spent while using Copy Out. 
  \par
  The transmission overhead is significant and Copy Out fares better for fewer records (1-2) of smaller sizes.
  \par
  }
  }
...
\framebox[0.9\textwidth][c]{
  \parbox{0.7\textwidth}{
  Copy Out crushes Zero Copy in terms of CPU savings for huge transmissions of big record sizes. 
  \par
  The transmission overhead for Zero Copy while transmitting fewer smaller sized records is more evident.
  \par
  It uses ~3x CPU cycles per byte to transmit a single 128B record over Zero Copy, Zero Copy starts winning after 3 records.
  \par
  }
  }
...
\framebox[0.9\textwidth][c]{
  \parbox{0.7\textwidth}{
  Copy Out has a larger memory footprint at bigger transmission sizes. The Zero Copy benefit is clear at 32~KB transmission. 
  \par
  For Zero Copy, the memory bandwidth follows a similar trend of transmission throughput.
  \par
...
\framebox[0.9\textwidth][c]{
  \parbox{0.7\textwidth}{
  The ratio clearly articulates why Zero Copy is a winner. It adds little overhead and remains consistent.
  \par
  For smaller records, there are configurations (3-4 records) where Copy Out yields similar transmission throughput and memory footprint.
  \par
...
\framebox[0.9\textwidth][c]{
  \parbox{0.7\textwidth}{
  We see the impact of DDIO is more prominent in Zero Copy since data is already present in LLC while using Copy-Out.
...
\framebox[0.9\textwidth][c]{
  \parbox{0.7\textwidth}{
  PCIe traffic offers a reasonable explanation for the sudden dip and subsequent trailing of Zero Copy performance for 16 or more 128~B records
  }
  } 
