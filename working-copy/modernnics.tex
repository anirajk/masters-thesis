...
server initiated.  We evaluated the performance of each of this using Infinibands verbs library. We implemented 
...
to work in pradtice negating the advantage of being one sided.
...
also an interesting measure to arrive at. Larger data sizes such as 1000~B recoreds showed a clearer benefit of using \cpp{SEND} in Figure~\ref{fig:1000B_transrate}.
\input{fig-200B-manymodes}
\input{fig-1000B-manymodes}
SEND almost always result in better throughput measurements than when we employ the other RDMA primitives.

\subsection{Record Sizes}
In the world of In-memory databases, there is an argument to be made that record sizes will get smaller since 
...
We choose 128~B records and 1024~B records for our evaluations in the rest of the thesis
